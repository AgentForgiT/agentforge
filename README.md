...
**1.0.0 shipped (2026-08-01)** — the stability line (DEC-0009): the API surface is frozen, every gate box audited with evidence (371 tests, 38 ADRs, docs complete, distribution automated, AICS Level-3), and the semver promise is public. **1.1.0 shipped (2026-08-08)** — MCP client mode (ADR-0039): the gateway calls remote MCP servers (`server.mcp_servers`, `mcp_<server>.<tool>` namespacing). Ecosystem companion repos (mcp, benchmarks, integrations, workflows, community) act as canonical-code reference hubs per DEC-0007.
